...
import { claims, expenses, categories, groups, memberships } from "../../lib/db/schema";
...
claim.post(
  ...
  async (c) => {
    ...
    if (expense.userId !== userId) return c.json(fail("FORBIDDEN", "Not your expense"), 403);
    ...
    if (existing) return c.json(fail("CONFLICT", "Expense already has a claim"), 409);
...
claim.get(
  "/",
  zValidator("query", claimQuerySchema),
  async (c) => {
    const userId = c.get("userId");
    ...
    if (query.groupId) {
      ...
      if (!membership) return c.json(fail("FORBIDDEN", "Not a member of this group"), 403);
...
claim.patch(
  "/:id/reject",
  zValidator("json", rejectClaimSchema),
  async (c) => {
    const id = c.req.param("id");
    const userId = c.get("userId");
    const body = c.req.valid("json");
    const [cl] = await db
      .update(claims)
      .set({ status: "rejected", reviewerId: userId, reviewNote: body.note ?? null, reviewedAt: new Date() })
      .where(eq(claims.id, id))
      .returning();
    if (!cl) return c.json(fail("NOT_FOUND", "Claim not found"), 404);
    return c.json(ok(cl));
