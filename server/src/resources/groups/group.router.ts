...
group.post('/', zValidator('json', createGroupSchema), async (c) => {
...
group.get('/company-summary', async (c) => {
    ...
    const userGroups = await db
        ...
        .innerJoin(memberships, eq(memberships.groupId, groups.id))
        .where(
            and(eq(groups.kind, 'department'), eq(memberships.userId, userId))
        )
        .orderBy(desc(groups.createdAt));
...
group.patch('/:id', zValidator('json', updateGroupSchema), async (c) => {
    const userId = c.get('userId');
    const id = c.req.param('id');
