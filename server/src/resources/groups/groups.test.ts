...
import { app, getTestUser, getSecondUser, getAuthHeaders } from "../../test/setup";
...
describe("Groups API", () => {
  ...
  describe("GET /api/groups", () => {
    ...
    it("returns groups the user belongs to", async () => {
      ...
      expect(body.data.some((g: any) => g.name === "My Group")).toBe(true);
    });
  });
  ...
  describe("POST /api/groups", () => {
    ...
    it("returns 201 and creates group with valid data", async () => {
      const user = getTestUser();
      const { res, body } = await createGroup(user.id, { name: "Fresh Group" });
    ...
    it("auto-adds creator as admin member", async () => {
      const user = getTestUser();
      const { body: group } = await createGroup(user.id, { name: "Admin Test" });
  ...
  describe("GET /api/groups/:id", () => {
    ...
    it("returns 403 when not a member", async () => {
      ...
      const { body: group } = await createGroup(user.id, { name: "Private Group" });
    ...
    it("returns 200 and group data for a member", async () => {
      const user = getTestUser();
      const { body: group } = await createGroup(user.id, { name: "Visible Group" });
  ...
  describe("PATCH /api/groups/:id", () => {
    ...
    it("returns 403 when not a member", async () => {
      const user = getTestUser();
      const secondUser = getSecondUser();
      const { body: group } = await createGroup(user.id, { name: "Protected Group" });
    ...
    it("returns 403 when a regular member (not admin)", async () => {
      const user = getTestUser();
      const secondUser = getSecondUser();
      const { body: group } = await createGroup(user.id, { name: "Member Group" });
    ...
    it("returns 200 and updates the group name (admin)", async () => {
      const user = getTestUser();
      const { body: group } = await createGroup(user.id, { name: "Renamable" });
  ...
  describe("DELETE /api/groups/:id", () => {
    ...
    it("returns 403 when not a member", async () => {
      const user = getTestUser();
      const secondUser = getSecondUser();
      const { body: group } = await createGroup(user.id, { name: "To Delete" });
    ...
    it("returns 403 when a regular member (not admin)", async () => {
      const user = getTestUser();
      const secondUser = getSecondUser();
      const { body: group } = await createGroup(user.id, { name: "Admin Only" });
    ...
    it("returns 200 and deletes the group (admin)", async () => {
      const user = getTestUser();
      const { body: group } = await createGroup(user.id, { name: "Delete Me" });
