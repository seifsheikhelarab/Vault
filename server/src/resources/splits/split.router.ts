...
split.post(
  "/",
  zValidator("json", createSplitSchema),
  async (c) => {
    ...
    if (Math.abs(total - Number(expense.amount)) > 0.01) {
      return c.json(fail("BAD_REQUEST", "Splits must equal expense amount"), 400);
    }
...
split.get(
  "/",
  zValidator("query", splitQuerySchema),
  async (c) => {
...
split.delete("/", async (c) => {
  ...
  if (!expenseId) return c.json(fail("BAD_REQUEST", "expenseId required"), 400);
