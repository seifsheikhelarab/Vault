...
function json(path: string, headers: Headers, method: string, body?: string): Promise<Response> {
...
function expenseItem(overrides: Partial<ExpenseItem> & { updatedAt: string }): ExpenseItem {
    return {
...
describe('POST /api/sync/push', () => {
    ...
    it('reports foreign ids as conflict-lost without touching them', async () => {
        ...
        expect(
            outcomeOf((await push(owner.headers, { expenses: [hijack] })).results, hijack.id),
        ).toBe('conflict-lost');
        expect(
    ...
    it('applies pushed tombstones and replays them as conflict-lost', async () => {
        ...
        expect(
            outcomeOf((await push(headers, { expenses: [tombstone] })).results, tombstone.id),
        ).toBe('accepted');
        expect(
    ...
    it('resolves category renames with the same LWW rule', async () => {
        ...
        expect((await t.db.category.findUniqueOrThrow({ where: { id: category.id } })).name).toBe(
            'Offline Cat',
        );
    ...
    it('validates item shape with the 422 envelope', async () => {
        ...
        for (const body of [
            ...
            {
                expenses: [
                    { id: crypto.randomUUID(), amountMinor: 1, occurredAt: fixedNow.toISOString() },
                ],
            }, // no updatedAt
            {
                expenses: [
...
describe('GET /api/sync/pull', () => {
    it('propagates deletions: tombstoned rows come back with deletedAt set', async () => {
        ...
        const goneRes = await json(
            '/api/expenses',
            headers,
            'POST',
            JSON.stringify({ id: crypto.randomUUID(), amountMinor: 200 }),
        );
    ...
    it('walks one watermark across both tables with stable pagination', async () => {
        ...
        await push(headers, {
            categories: [
                ...
                {
                    id: crypto.randomUUID(),
                    updatedAt: new Date(fixedNow.getTime() + 3500).toISOString(),
                    name: 'C2',
                },
            ],
            expenses: items,
        ...
        do {
            ...
            seenIds.push(
                ...page.expenses.map((e) => e.id as string),
                ...page.categories.map((c) => c.id),
            );
