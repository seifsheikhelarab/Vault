...
type DashboardBody = {
  ...
  budgets: { id: string; periodType: 'week' | 'month'; categoryId: string | null; spent: number; limit: number; pct: number }[]
  recentExpenses: { id: string; amountMinor: number; currency: string; categoryId: string | null; occurredAt: string; note: string | null }[]
}

async function getDashboard(path: string, headers: Headers): Promise<{ res: Response; body: DashboardBody }> {
...
describe('GET /api/dashboard', () => {
  it('returns the zero shape on an empty account', async () => {
    ...
    const { res, body } = await getDashboard('/api/dashboard?date=2026-02-18T12:00:00Z', headers)
  ...
  it('includes each budget spent-vs-limit progress for its current period', async () => {
    ...
    await t.request('/api/budgets', {
      method: 'POST',
      headers: (() => { const h = new Headers(headers); h.set('content-type', 'application/json'); return h })(),
    ...
    await t.request('/api/budgets', {
      method: 'POST',
      headers: (() => { const h = new Headers(headers); h.set('content-type', 'application/json'); return h })(),
      body: JSON.stringify({ periodType: 'week', amountMinor: 50000, categoryId: groceries.id }),
    })
    ...
    await createExpense(headers, { amountMinor: 7000, occurredAt: '2026-02-17T12:00:00Z', categoryId: groceries.id })

    const { body } = await getDashboard('/api/dashboard?date=2026-02-18T12:00:00Z', headers)
    expect(body.budgets).toEqual([
      { id: expect.any(String), periodType: 'month', categoryId: null, spent: 37000, limit: 100000, pct: 37 },
      { id: expect.any(String), periodType: 'week', categoryId: groceries.id, spent: 7000, limit: 50000, pct: 14 },
    ])
  })
  ...
  it('lists the 5 most recent expenses newest-first, excluding tombstones', async () => {
    ...
    for (let day = 1; day <= 7; day++) {
      ids.push(await createExpense(headers, { amountMinor: day * 1000, occurredAt: `2026-02-0${day}T12:00:00Z`, ...(day === 6 && { note: 'latte' }) }))
    }
    // Remove the newest and an older middle entry.
    expect((await t.request(`/api/expenses/${ids[6]}`, { method: 'DELETE', headers })).status).toBe(204)
    expect((await t.request(`/api/expenses/${ids[2]}`, { method: 'DELETE', headers })).status).toBe(204)

    const { body } = await getDashboard('/api/dashboard?date=2026-02-18T12:00:00Z', headers)
    expect(body.recentExpenses.map((e) => e.id)).toEqual([ids[5], ids[4], ids[3], ids[1], ids[0]])
