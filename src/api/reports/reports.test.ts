...
async function getReport(path: string, headers: Headers): Promise<{ res: Response; body: ReportBody }> {
...
describe('GET /api/reports/monthly', () => {
  it('totals live expenses per category with previous-month delta', async () => {
    ...
    await createExpense(headers, { amountMinor: 12000, occurredAt: '2026-02-20T12:00:00Z', categoryId: transport.id })
    ...
    // Previous month only.
    await createExpense(headers, { amountMinor: 100000, occurredAt: '2026-01-20T12:00:00Z', categoryId: groceries.id })

    const { res, body } = await getReport('/api/reports/monthly?date=2026-02-18T12:00:00Z', headers)
    expect(res.status).toBe(200)
    expect(body.period).toEqual({ start: '2026-01-31T22:00:00.000Z', end: '2026-02-28T22:00:00.000Z' })
...
describe('GET /api/reports/weekly', () => {
  it('splits expenses across the Monday boundary on the Cairo calendar', async () => {
    ...
    await createExpense(headers, { amountMinor: 7000, occurredAt: '2026-02-15T22:05:00Z', categoryId: groceries.id })
    await createExpense(headers, { amountMinor: 20000, occurredAt: '2026-02-19T10:00:00Z', categoryId: transport.id })

    const { res, body } = await getReport('/api/reports/weekly?date=2026-02-18T12:00:00Z', headers)
    expect(res.status).toBe(200)
    expect(body.period).toEqual({ start: '2026-02-15T22:00:00.000Z', end: '2026-02-22T22:00:00.000Z' })
...
describe('empty periods and zero baselines', () => {
  it('returns zeros and null deltaPct when nothing exists', async () => {
    ...
    const { res, body } = await getReport('/api/reports/monthly?date=2030-06-15T12:00:00Z', headers)
    expect(res.status).toBe(200)
