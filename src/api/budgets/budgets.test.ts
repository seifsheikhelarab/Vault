...
describe('POST /api/budgets', () => {
    ...
    it.each([
        ...
        [
            'unknown field value for category',
            { periodType: 'month', amountMinor: 100, categoryId: 'nope' },
        ],
...
describe('GET /api/budgets/progress', () => {
    ...
    async function setup(): Promise<Setup> {
        ...
        return {
            headers,
            overall,
            groceryBudget: { ...groceryBudget, categoryId: groceries.id },
            weekly,
        };
    }
    ...
    it('computes spent/limit/pct per active budget at the given date', async () => {
        ...
        // 20+8+7 = 35k of 20k.
        expect(byId.get(s.weekly.id)).toMatchObject({
            spent: 35000,
            limit: 20000,
            pct: 175,
            periodType: 'week',
    ...
    it('accepts date-only and full datetime forms identically', async () => {
        ...
        const b = await t.request(`/api/budgets/progress?date=${encodeURIComponent(DATE)}`, {
            headers: s.headers,
        });
