...
describe('POST /api/chat/parse', () => {
    it('returns a bare draft with the category guess resolved case-insensitively', async () => {
        ...
        const seeded = (await (await t.request('/api/categories', { headers })).json()) as {
            id: string;
            name: string;
        }[];
        ...
        const chat = buildApp({
            parseExpense: async () => taxiDraft({ categoryGuess: 'transport' }),
        });
        const res = await parse(
            chat,
            headers,
            JSON.stringify({ message: 'taxi 120 to airport yesterday' }),
        );
