...
function ClaimsQueue() {
    ...
    useEffect(() => {
        const idx = STATUS_TABS.indexOf(
            statusFilter as (typeof STATUS_TABS)[number]
        );
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                    {[1, 2, 3, 4].map((i) => (
                        <div
                            key={i}
                            className="h-20 bg-surface rounded-[16px] animate-pulse shadow-warm-sm border border-border-light"
                        />
                    ))}
                </div>
            </div>
                ...
                {filtered.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {filtered.map((claim: ClaimWithExpense, i: number) => {
                            const catName =
                                catMap.get(claim.expense.categoryId) ?? 'Other';
                            const isMyClaim =
                                currentUserId &&
                                claim.expense.userId === currentUserId;
                            return (
                                <div
                                    key={claim.id}
                                    ...
                                    style={{
                                        opacity: revealed ? 1 : 0,
                                        transition: `opacity 0.4s cubic-bezier(0.22, 1, 0.36, 1) ${0.2 + i * 0.04}s`
                                    }}
                                >
                                    <div className="flex items-start justify-between gap-4">
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            <span
                                                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${STATUS_COLORS[claim.status] ?? ''}`}
                                            >
                                                ...
                                                viewMode === 'admin' && (
                                                    <div className="flex items-center gap-1.5 ml-2">
                                                        ...
                                                        <Button
                                                            ...
                                                            size="sm"
                                                            variant="ghost"
                                                            className="bg-error/10 text-error border border-error/20 hover:bg-error/20"
                                                        >
                                                            Reject
                                                        </Button>
                                                    </div>
                                            ...
                                            {claim.status === 'approved' &&
                                                viewMode === 'admin' && (
                                                    <Button
                                                        onClick={() =>
                                                            reimburseClaim.mutate(
                                                                claim.id
                                                            )
                                                        }
                                                        disabled={
                                                            reimburseClaim.isPending
                                                        }
                                                        size="sm"
                                                        variant="ghost"
                                                        className="bg-info/10 text-info border border-info/20 hover:bg-info/20"
                                                    >
                                                        Mark Reimbursed
                                                    </Button>
                                                )}
